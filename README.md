...
- **Nginx hardening** — rate limiting on wp-login.php, HSTS, CSP-RO, wp-config/uploads/xmlrpc deny, FastCGI cache (optional), Brotli (optional)
- **Fail2Ban** — auto-configured WordPress brute-force protection
- **Auto-renewal** — systemd daily timer with randomized delay, `--cert-name` precision renewal
...
- **Nginx 加固** — wp-login.php 速率限制、HSTS、CSP-RO、wp-config/uploads/xmlrpc 拦截、FastCGI 缓存（可选）、Brotli（可选）
- **Fail2Ban** — 自动配置 WordPress 暴力破解防护
- **自动续期** — systemd 每日定时器，随机延迟，`--cert-name` 精准续期
